	...
	closeOtherPage:function(){
		...
		for(var i=1;i<multitaskVar.hasOpenMenu.length;i++){
			if(multitaskVar.hasOpenMenu[i] != undefined&&multitaskVar.hasOpenMenu[i]!=null){
				...
				if(url.indexOf("prjId")==-1){
					if(url.indexOf("?")==-1){
						url+='?prjId='+prjId+'&contractId='+contractId;
					}else{
						url+='&prjId='+prjId+'&contractId='+contractId;
					}
				}else{
					if(url.indexOf("&isCrossDomain")!=-1){
						...
						url=url.split("prjId")[0];
						url+='prjId='+prjId+'&contractId='+contractId+'&isCrossDomain='+str;
					}else{
						if(url.indexOf("&xmgl")!=-1){
							var index=url.lastIndexOf("&xmgl");
							var str=url.substring(index+6,url.length);
							url=url.split("prjId")[0];
							url+='prjId='+prjId+'&contractId='+contractId+'&xmgl='+str;
						}else{
							url=url.split("prjId")[0];
							url+='prjId='+prjId+'&contractId='+contractId;
						}
...
//		multitaskVar.contextMenu.css("display","none");
	//},
	closeMenuPageById : function(menuID) {
		var id=multitaskVar.hasOpenMenu[menuID].code;
		$("#"+id).removeClass("selected");
		this.closePage(multitaskVar.hasOpenMenu[menuID]);
		multitaskVar.contextMenu.css("display","none");
		var objs=$("div.multitask-item-separate");
		for(var i=0;i<objs.length;i++){
			if($(objs[i]).attr("isonclick")=="true"){
				var id2=$(objs[i]).attr("code");
	...
	addMenuTab : function(newMenuCode, newMenuTitle, newMenuUrl,flag,datawrap,defaultCloseFirstTask,appcontext) {
		...
		if(newMenuUrl.indexOf("helpCode")<0){
          ...
          if(appcontext != undefined && appcontext != ""){ //有appcontext是通过openTabTask方法调用的
           if(length != -1){
		...
		if (hasTabNum == multitaskVar.maxNum) {
		   ...
		   if(taskDirectClosable=="true"){
			 ...
			 if("Y" == localStorage.defaultCloseFirstTask || localStorage.defaultCloseFirstTask == true) {
			 	submit();
			 } else {
...
function freshPage(e,id){
	e.stopPropagation();
	document.getElementById(id+"_bodyFrame").contentWindow.location.reload(true);
}
